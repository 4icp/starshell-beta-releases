(function() {
  ...
  const { text_to_buffer } = function(exports) {
    ...
    const deriveHdPath = (crypto2, node, path) => {
      ...
      return isPrivateDerivation ? indexes.reduce(
        (result, nextIndex) => typeof result === "string" ? result : deriveHdPrivateNodeChild(crypto2, result, nextIndex),
        node
      ) : indexes.reduce(
        (result, nextIndex) => typeof result === "string" ? result : deriveHdPublicNodeChild(crypto2, result, nextIndex),
        node
      );
    };
    ...
    const mergeRanges = (ranges, parentRange = {
      ...
      const unsortedMerged = ranges.length < minimumRangesToMerge ? ranges.length === 1 ? ranges[0] : parentRange : ranges.reduce(
        (merged, range2) => ({
    ...
    const getSigningSerializationType = (algorithmIdentifier, prefix = "") => {
      switch (algorithmIdentifier) {
        ...
        case `${prefix}${SigningSerializationAlgorithmIdentifier.correspondingOutput}`:
          return Uint8Array.of(
            SigningSerializationFlag.correspondingOutput | SigningSerializationFlag.forkId
          );
        case `${prefix}${SigningSerializationAlgorithmIdentifier.correspondingOutputSingleInput}`:
          return Uint8Array.of(
            SigningSerializationFlag.correspondingOutput | SigningSerializationFlag.singleInput | SigningSerializationFlag.forkId
          );
        ...
        case `${prefix}${SigningSerializationAlgorithmIdentifier.noOutputsSingleInput}`:
          return Uint8Array.of(
            SigningSerializationFlag.noOutputs | SigningSerializationFlag.singleInput | SigningSerializationFlag.forkId
          );
        default:
          return void 0;
      }
  ...
  function deep_freeze(z_object, h_frozen = /* @__PURE__ */ new Map()) {
    if ("object" !== typeof z_object && "function" !== typeof z_object)
      return;
    if (!z_object)
      return;
  ...
  function normalize_stack(a_lines, i_start = 1) {
    return [
      a_lines[0],
...
${e_post.stack}`));
  ...
  }
  {
    ...
    if ("object" !== typeof gc_confirm || false !== gc_confirm.configurable || f_getter_starshell !== gc_confirm.get || f_setter_starshell !== gc_confirm.set) {
